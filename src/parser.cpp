...
std::unique_ptr<ExprNode> Parser::parse_block() {
    ...
    do {
        ...
        if (!match(TokenType::Punctuation, ";")) {
            ...
            throw std::runtime_error(
                "[squaker.parser.block] Expected ';' or '}' after expression" + context);
        }
...
std::unique_ptr<ExprNode> Parser::parse_if_expression() {
    std::vector<std::pair<std::unique_ptr<ExprNode>,
        std::unique_ptr<ExprNode>>> branches;
...
std::unique_ptr<ExprNode> Parser::parse_array() {
    ...
    // 期望右方括号
    if (!match(TokenType::Punctuation, "]")) {
        std::string context = current < tokens.size() ? 
            " at token '" + tokens[current].value + "'" : "";
        throw std::runtime_error(
